...
Class
    Foam::RASModels::kOmegaSST_nutML_compressible

SourceFiles
    kOmegaSST_nutML_compressible.C

\*---------------------------------------------------------------------------*/

#include "kOmegaSST_nutML_compressible.H"
...
defineTypeNameAndDebug(kOmegaSST_nutML_compressible, 0);
addToRunTimeSelectionTable(RASModel, kOmegaSST_nutML_compressible, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //


tmp<volScalarField> kOmegaSST_nutML_compressible::F1
(
...
tmp<volScalarField> kOmegaSST_nutML_compressible::F2() const
{
...
tmp<volScalarField> kOmegaSST_nutML_compressible::F3() const
{
...
tmp<volScalarField> kOmegaSST_nutML_compressible::F23() const
{
...
void kOmegaSST_nutML_compressible::correctNut
(
...
tmp<volScalarField> kOmegaSST_nutML_compressible::epsilonByk
(
...
tmp<fvScalarMatrix>
kOmegaSST_nutML_compressible::kSource() const
{
...
tmp<fvScalarMatrix>
kOmegaSST_nutML_compressible::omegaSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
tmp<fvScalarMatrix> kOmegaSST_nutML_compressible::Qsas
(
    const volScalarField& S2,
...
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //
kOmegaSST_nutML_compressible::kOmegaSST_nutML_compressible
(
...
:
    eddyViscosity<compressible::RASModel>
    (
        type,
        alpha,
        rho,
...
bool kOmegaSST_nutML_compressible::read()
{
    if (eddyViscosity<compressible::RASModel>::read())
    {
...
tmp<fvVectorMatrix> kOmegaSST_nutML_compressible::divDevRhoReff(volVectorField& U) const
{
...
void kOmegaSST_nutML_compressible::correct()
{
    dimensionedScalar smallnutU_("smallnutU", dimensionSet(0, 2, -1, 0, 0, 0, 0), 2e-08);
    dimensionedScalar smallUsquare_(" smallUsquare", dimensionSet(0, 2, -2, 0, 0, 0, 0), 1e-08);
    dimensionedScalar smallOmega_("smallOmega", dimensionSet(0, 0, -1, 0, 0, 0, 0), 1e-08);
    ...
    eddyViscosity<compressible::RASModel>::correct();
